...
Setup:
1. Create large DAG (this is the shared model), given a fixed number of repeating cells.
2. Create controller RNN, configure its output to generate a subsampled DAG from the large DAG

Training Loop
    First phase : train shared parameters of DAF for 400 steps, each on minibatch of 64 examples
        ...
    Second phase : train controller paramters for 2000 steps.
...
class Trainer(Trainable):
    ...
    def setup(self, config):
        ...
        dataset = Corpus(self.args.data_path)
        ...
        for self.epoch in range(self.start_epoch, self.args.max_epoch):
            ...
            if self.epoch % self.args.save_epoch == 0:
                with _get_no_grad_ctx_mgr():
                    ...
                    val_loss, ppl = self.evaluate(self.eval_data,
                                  best_dag,
                                  'val_best',
    ...
    def derive(self, sample_num=None, valid_idx=0):
        ...
        path = os.path.join(self.logdir, 'networks', fname)
    ...
    def shared_path(self):
        ...
        return f'{self.logdir}/shared_epoch{self.epoch}_step{self.shared_step}.pth'

    @property
    def controller_path(self):
        # return f'{self.args.model_dir}/controller_epoch{self.epoch}_step{self.controller_step}.pth'
        return f'{self.logdir}/controller_epoch{self.epoch}_step{self.controller_step}.pth'

    def get_saved_models_info(self):
        paths = glob.glob(os.path.join(self.logdir, '*.pth'))
        paths.sort()
    ...
    def save_model(self):
        ...
        for epoch in epochs[:-self.args.max_save_num]:
            paths = glob.glob(
                os.path.join(self.logdir, f'*_epoch{epoch}_*.pth'))
...
def run_experiment(main_args):
    ...
    if main_args.smoke_test:
        args.num_blocks = 2

    if main_args.ray_address:
        ray.init(address=main_args.ray_address)
    ...
    analysis = tune.run(
        ...
        stop={
            "mean_accuracy": 0.95,
            "training_iteration": 2 if main_args.smoke_test else 100
        },
        resources_per_trial={
            "cpu": 2,
            "gpu": 1  # int(args.cuda) * 0.5
        },
        num_samples=1 if main_args.smoke_test else 50,
        config={
            "args": args
        }
    )

    print("Best config is:", analysis.get_best_config(metric="val_loss"))

if __name__ == "__main__":
    import sys
    sys.path.append("..")

    parser = argparse.ArgumentParser(description="ENAS: RNN NAS")
